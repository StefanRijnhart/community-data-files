...
class AdrGoods(models.Model):
    ...
    classification_code_id = fields.Many2one(
        comodel_name="adr.classification.code", string="Classification Code"
    )
    ...
    _sql_constraints = [
        (
            "un_number_unique",
            "unique(un_number)",
            "A dangerous goods with this UN Number already exists",
        )
    ]
    ...
    @api.constrains("class_id", "classification_code_id")
    def _check_classification_code(self):
        for goods in self:
            if (
                goods.class_id.classification_code_required
                and not goods.classification_code_id
            ):
                raise ValidationError(
                    _("A classification code is required for goods with class %s")
                    % (goods.class_id.code)
                )
    ...
    def name_get(self):
        """Prepend the UN Number to the class name"""
        return [(rec.id, "{} {}".format(rec.un_number, rec.name)) for rec in self]
